...
## 6. 현재 상태 (2026-08-17 갱신 — v0.7.0)

### v0.7.0 변경 (완료)
- **메가박스·롯데시네마 전 극장 추가.** 3사 합계 **426개 극장 · 2,798개 관 · 364,255석**.
  - 메가박스 115개 극장 730개 관 84,603석 · 롯데시네마 133개 극장 872개 관 121,815석 (미수집 0).
  - 수집은 Node 스크립트(`tools/collect-megabox.js`·`tools/collect-lotte.js`)로 공개 예매 API 를
    **로그인·쿠키 없이 읽기 전용** 조회. 예매·결제 행위 없음. 동시 요청 4개 · 증분 재실행 가능.
  - 브라우저에서 수집해 로컬로 넘기는 방식은 폐기했다 — https 페이지에서 http://127.0.0.1 로 보내는
    요청이 혼합 콘텐츠로 차단되고(sendBeacon 도 조용히 실패), 도구 출력은 1 KB 남짓에서 잘려
    수십만 자를 되가져올 수 없었다. Node 에서 직접 호출하니 세 API 모두 인증 없이 응답한다.
- **스키마 v3 — 브랜드 계층 추가**: `THEATER_DATA.brands[] → regions[] → theaters[] → screens[]`.
  siteNo 에 브랜드 접두사를 둬 좌석 파일이 섞이지 않게 했다 (CGV `0056` · 메가박스 `mb1372` ·
  롯데 `lc1013`). 관 단위 `sourceNote` 는 브랜드 공통 문구로 접어 인덱스를 581 KB 로 줄였다.
- **좌표계 정규화**: 세 사업자 좌표계가 다르다 (CGV 그리드 · 메가박스 행열 그리드 · 롯데 픽셀).
  `tools/import-raw.js` 의 `encodeGrid()` 가 열 안 이웃 좌석 간격의 **최빈값**으로 피치를 역산해
  기존 CGV 그리드(1단위 0.28 m, 좌석 2단위)로 통일한다. 최솟값이 아니라 최빈값을 쓰는 이유는
  좌표가 겹치는 이상 좌석이 실제로 존재하기 때문. 전 2,798관 무손실 검증(좌석수 불일치 0).
- **출입구 실좌표 렌더링**: 메가박스 `gateTyCd`, 롯데 `Enterences` 가 좌석도와 같은 좌표계로
  출입구를 준다 (1,481관 2,516곳). `layout.js`가 미터로 변환하고 렌더러가 그 자리에 문·유도등을
  둔다. **종류 코드는 유도표지 화살표 방향이지 벽면이 아니다** (같은 오른쪽 벽 두 문이 GTR·GTL 로
  갈린다) — 그래서 앞/뒤·좌/우는 좌석 블록 대비 좌표로 판정한다. CGV 는 좌표 미공개라 기존
  소방 기준 표준 배치를 유지한다. 좌석 평면도에도 EXIT 표기가 함께 그려진다.
- **특별관 기하 추정 세분화** (`estimate.js` 의 `HALL` 표): imax·screenx·superplex·dolby·premium·
  led·4dx·boutique·recliner·comfort 별로 스크린 폭 비율·종횡비·최전열 거리·열 피치·곡률을 분리.
  스크린 폭 하한을 7 m → 4.5 m 로 낮췄다 — 8~30석 부티크·스위트관까지 다루게 되면서 옛 하한이
  소형관을 왜곡했다.
- **실측 큐레이션은 늘리지 않았다**: 메가박스·롯데는 관별 실측치를 공개하지 않고, 홍보 문구로 도는
  수치(월드타워 수퍼플렉스 "34 m")는 수집한 좌석 배치(해당 관 객석 최대 폭 25.8 m)와 맞지 않아
  실측으로 올리지 않았다. 여전히 "실측"은 용산 IMAX 하나뿐.
- UI: 패널 최상단 브랜드 탭(선택된 브랜드만 사업자 색), 좌석 평면도의 선택 좌석·범례가 브랜드 색을
  따르고 NOTE 에 출입구 출처를 명시.
- `tools/import-cgv-raw.js` 는 3사 통합 `tools/import-raw.js` 로 대체(삭제).

### 그 이전

### v0.3.0 변경 (완료 — Release v0.3.0 게시)
- 객석 단차: 고정 상수 → **C-value 시야선 역산** (눈→스크린 하단 시선이 엇배열 2열 전방 머리끝 +0.12 m 위를 지나도록, 중간 열 기준 고정점 반복, 클램프 0.26~0.45 m). 관 형태별로 달라짐(예: 일반 소형관 0.44, IMAX 0.35). 용산 IMAX 는 큐레이션 0.45 유지. 근거·수식은 src/data/estimate.js 주석.
- 스크린 사실성 4종 (renderer.stub + CODEX_PROMPT §6 5~8항): 시선각 게인 감쇠(실린더 법선 기준 최대 30%), 점등 가장자리 마스킹 그림자, 스크린 앞 바닥 반사광(α5%), 커튼 산란광 스필(α5%).

### v0.2.2 변경 (완료 — Release v0.2.2 게시)
- 측면 좌석에서 곡면 스크린 위 검은 세로선/판 제거: 커튼·전방 비상구를 스크린보다 먼저 그리도록 물리적 가림 순서로 재배치.

### v0.2.1 변경 (완료 — Release v0.2.1 게시)
- 스크린 좌우 가장자리 검은 세로선 제거: 마스킹·점등 영역을 이음새 없는 단일 경로로 그리고, 텍스처 삼각형을 픽셀 단위(1.5px)로 확장해 이웃과 겹치게 함.
- SCREENX 정면-측면 연결부: 측면 시작점을 점등 모서리의 실제 3D 좌표에 일치시키고 점감광(15%→55%)으로 끊김·밝기 단차 제거.
- 비상구: ISO 7010 달리는 사람 픽토그램 유도등 + 문 실루엣. 표준 위치 반영(스크린 양측 전방 출구 2곳 + 객석 후방 측면 출입구 2곳 — CGV 는 관별 비상구 좌표 미공개이므로 소방 기준 표준 배치. 데이터에 좌표가 생기면 그것 우선). CODEX_PROMPT 에도 동일 요구 반영.
...
- **버그 수정**: file:// 캔버스 오염 시 capture() SecurityError → try/catch, 기본 포맷 formats[0] 통일(용산 IMAX 는 1.90 우선 순서), 지역 라벨 "N개관"→"극장 N".
- 포스터를 사용자 제공 아바타 스틸(3840×2160)로 교체.

### v0.1.0 (완료분)
- **플랫폼 완성, 스텁 렌더러로 전체 동작. 남은 것은 Codex 의 3D 본 렌더러.**
- 사용자 결정 사항:
...
- 포스터: 사용자 제공 이미지를 assets/poster-odyssey.jpg 로 배치. 사용자 결정으로 공개 저장소·배포 zip·웹 버전에 모두 포함 (Pages 에서 표시 확인 완료).
