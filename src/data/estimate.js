 ...
 * (theaters.json 의 screen/auditorium 이 null, geometrySource === "estimated")은
 * 좌석 배치(열 수·좌석 수·그리드 폭)로부터 이 함수가 기하를 추정한다.
 * 추정값은 UI 에 "추정" 으로 표기된다. 추측을 실측인 척 하지 않는다.
 *
 * 추정 근거(모든 상수는 국내 멀티플렉스 통상 범위):
 *  - 1 그리드단위 = 0.28 m (좌석 피치 0.56 m 의 절반, import-cgv-raw.js 와 동일)
...
(function () {
  ...
  /**
   * @param {Object} screenRec theaters.json 의 상영관 레코드 (rows·grid 필요)
   * @returns {{screen:Object, auditorium:Object}} §3.2 형태
   */
  function estimateScreenGeometry(screenRec) {
    var name = screenRec.name || "";
    var kinds = {};
    (screenRec.rows || []).forEach(function (r) {
      r.seats.forEach(function (s) { kinds[s.grade] = true; });
    });
    var isImax = /IMAX/i.test(name);
    var is4dx = /4DX/i.test(name);
    var isScreenX = /SCREENX/i.test(name);
    var isRecliner = Object.keys(kinds).some(function (k) { return /리클라이너/.test(k); });
    var isComfort = Object.keys(kinds).some(function (k) { return /컴포트/.test(k); });

    var g = screenRec.grid;
    var rowWidthM = (g.maxX - g.minX) * UNIT_M;
    ...
    return {
      screen: {
        ...
        curvatureRadiusM: null,   // 추정관은 평면으로 둔다 (곡률 근거 없음)
        tiltDeg: 0,
        maskingRatios: {}
      },
      auditorium: {
