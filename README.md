...
- 세 사업자의 좌표계가 서로 다르므로(CGV 그리드 · 메가박스 행열 그리드 · 롯데시네마 픽셀)
  임포터가 좌석 간격의 최빈값으로 피치를 역산해 하나의 그리드로 정규화한다. 일반석 가로 1단위는
  0.28m이고, 리클라이너·부티크는 실제 좌석 폭 범위에 맞춰 가로 단위를 확장한다.
...
- **스크린 실측치**: 세 사업자 모두 관별 치수를 대체로 공개하지 않는다. 공개 수치가 확인되는
  용산 IMAX(31×22.4m)와 월드타워 SUPER PLEX(34×13.8m)는 "실측"이고 나머지는 좌석 배치 + 특별관 종류 기반 **추정**
...
- 원본 영화 장면은 확대 크롭하지 않고 화면 안에 원본 비율로 맞춘다. 포맷과 원본 화면비가 다르면
  남는 영역은 검은 마스킹으로 보이며, 화면 광량은 모니터 기준의 상대 밝기 시뮬레이션이다.
- 좌석 종합 등급은 화면각·올림각·이탈각뿐 아니라 앞사람 머리와 화면 하단 시선 사이의 C-value를
  포함한다. 200mm 이상 `GOOD`, 150–200mm `허용`, 120–150mm `POOR`, 120mm 미만 `가림 위험`이다.
- 파이프라인:
...
- 화면 위 요약은 좌석 등급과 권장 시야 충족 여부를 즉시 보여 주며, 하단 계측표에 C-value까지 표시한다.
  리클라이너·빈백·소파베드는 좌석 폭과 착석 눈높이를 별도로 반영한다.
